 ...
 *
 * On NeoForge the builder transformer methods attach custom OBJ/JSON model
 * providers to the Registrate builder. On Fabric, Registrate-Refabricated is
 * provided transitively by Create Fly, so the same {@code BlockBuilder} types
 * are available. However, most NeoForge-specific model providers (OBJ loaders,
 * custom blockstate generators) do not exist on Fabric.
 *
 * <p>These implementations therefore return the builder unchanged (identity).
 * Block and item registration still works correctly; the blocks just use the
 * model JSON files that are already present under
 * {@code src/main/resources/assets/railways/models/}. Custom renderer logic
 * (e.g. FuelTankModel) is registered separately in the client entrypoint.
 */
