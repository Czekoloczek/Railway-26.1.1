	...
	public static ConductorCapModel<?> of(ItemStack stack, HumanoidModel<?> base, LivingEntity entity) {
		if (defaultModel == null) {
			...
			CRBlockPartials.CUSTOM_CONDUCTOR_CAPS.forEach((name, partial) -> {
				String normalizedName = CRBlockPartials.normalizeCustomCapName(name);
				ConductorCapModel<?> model = new ConductorCapModel<>(root, partial, CRBlockPartials.shouldPreventTiltingCap(name));
				customModels.put(Pair.of(normalizedName, false), model); // normal caps should apply whether they are on a conductor
				customModels.put(Pair.of(normalizedName, true), model);
			});
			CRBlockPartials.CUSTOM_CONDUCTOR_ONLY_CAPS.forEach((name, partial) -> {
				String normalizedName = CRBlockPartials.normalizeCustomCapName(name);
				ConductorCapModel<?> model = new ConductorCapModel<>(root, partial, CRBlockPartials.shouldPreventTiltingCap(name));
				Pair<String, Boolean> key = Pair.of(normalizedName, true); // for conductors, override conductor model
				customModels.put(key, model);
			});
			defaultModel = new ConductorCapModel<>(root, null, false);
		}
		String name = CRBlockPartials.normalizeCustomCapName(stack.getHoverName().getString());
